...
import model.Block;
import model.Colt;
import model.ColtBullet;
import model.ColtSuper;
import model.ElixirBox;
import model.Gem;
import model.Pam;
import model.PamBullet;
import model.PamSuper;
import model.Poison;
import model.Triple;
import view.*;
...
public class Controller {
    ...
    private void generateElixirBoxes() {
        int boxSize = 10; // Define based on your game logic
    ...
    private void play() {
        ...
        while (true) {
            ...
            for (int ev = 0; ev < 2; ev++) {
                for (int i = 0; i < 15; i++) {
                    for (int j = 0; j < 16; j++) {
                        if ((((i * 100 + (ev % 2) * 50)) < (distX / 2))
                                || (((i * 100 + (ev % 2) * 50) + 50) > (1450 - (distX / 2))) || ((j * 60) < distY / 2)
                                || ((j * 60) > (840 - distY / 2))) {
                            if ((ev % 2 == 0) && (j % 2 == 0)) {
            ...
            if (((pamammo > 0) && pam.shoot() || pamShot)) {
                ...
                if (pamShotCounter == 0) {
                    ...
                    pamInitialX = pam.getX();
                    pamInitialY = pam.getY();

                    pShots.add(new PamBullet(pam.getX(), pam.getY(), pamangle));
                    pShotsInitial.add(new Triple(pam.getX(), pam.getY(), pamangle));

                    pShots.add(
                            new PamBullet(pam.getX(), pam.getY(), pamangle + 5 - (int) (1 - (pamangle % 360) / 180)));
                    pShotsInitial
                            .add(new Triple(pam.getX(), pam.getY(), pamangle + 5 - (int) (1 - (pamangle % 360) / 180)));

                    pShots.add(new PamBullet(pam.getX(), pam.getY(), pamangle - 5 + (int) ((pamangle % 360) / 180)));
                    pShotsInitial
                            .add(new Triple(pam.getX(), pam.getY(), pamangle - 5 + (int) ((pamangle % 360) / 180)));
                    pamShotCounter++;
                    ...
                    pShots.add(
                            new PamBullet(pam.getX(), pam.getY(), pamangle + 5 - (1 - (int) (pamangle % 360) / 180)));
                    pShotsInitial
                            .add(new Triple(pam.getX(), pam.getY(), pamangle + 5 - (1 - (int) (pamangle % 360) / 180)));

                    pShots.add(new PamBullet(pam.getX(), pam.getY(), pamangle - 5 + (int) (pamangle % 360) / 180));
                    pShotsInitial.add(new Triple(pam.getX(), pam.getY(), pamangle - 5 + (int) (pamangle % 360) / 180));
            ...
            if (((coltammo > 0) && colt.shoot()) || coltShot) {
                ...
                if (coltShotCounter == 0) {
                    ...
                    coltInitialX = colt.getX();
                    coltInitialY = colt.getY();
                    ...
                    cShots.add(new ColtBullet(
                            coltInitialX + (int) (20 * Math.sin(2 * Math.PI * ((coltInitialAngle) / 360.0)))
                            + (int) (20 * Math.cos(2 * Math.PI * ((coltInitialAngle) / 360.0))),
                            coltInitialY + (int) (20 * Math.cos(2 * Math.PI * ((coltInitialAngle) / 360.0)))
                            - (int) (20 * Math.sin(2 * Math.PI * ((coltInitialAngle) / 360.0))),
                            coltInitialAngle));
                    cShotsInitial.add(new Triple(
                            coltInitialX + (int) (20 * Math.sin(2 * Math.PI * ((coltInitialAngle) / 360.0)))
                            + (int) (20 * Math.cos(2 * Math.PI * ((coltInitialAngle) / 360.0))),
                            coltInitialY + (int) (20 * Math.cos(2 * Math.PI * ((coltInitialAngle) / 360.0)))
                            - (int) (20 * Math.sin(2 * Math.PI * ((coltInitialAngle) / 360.0))),
            ...
            if ((colt.coltSuper() && cSuper >= 100) || coltSuper) {
                ...
                if (coltSuperCounter == 0) {
                    ...
                    coltSuperInitialX = colt.getX();
                    coltSuperInitialY = colt.getY();
                    ...
                    cSupers.add(new ColtSuper(
                            coltSuperInitialX + (int) (20 * Math.sin(2 * Math.PI * ((coltSuperInitialAngle) / 360.0)))
                            + (int) (20 * Math.cos(2 * Math.PI * ((coltSuperInitialAngle) / 360.0))),
                            coltSuperInitialY + (int) (20 * Math.cos(2 * Math.PI * ((coltSuperInitialAngle) / 360.0)))
                            - (int) (20 * Math.sin(2 * Math.PI * ((coltSuperInitialAngle) / 360.0))),
                            coltSuperInitialAngle));
                    cSuperInitial.add(new Triple(
                            coltSuperInitialX + (int) (20 * Math.sin(2 * Math.PI * ((coltSuperInitialAngle) / 360.0)))
                            + (int) (20 * Math.cos(2 * Math.PI * ((coltSuperInitialAngle) / 360.0))),
                            coltSuperInitialY + (int) (20 * Math.cos(2 * Math.PI * ((coltSuperInitialAngle) / 360.0)))
                            - (int) (20 * Math.sin(2 * Math.PI * ((coltSuperInitialAngle) / 360.0))),
            ...
            for (int i = 0; i < pShots.size(); i++) {
                for (int j = 0; j < elixirBoxes.size(); j++) {
                    if ((i < pShots.size()) && pShots.get(i).checkCollisionBox(elixirX.get(j), elixirY.get(j))
                            && (Math.sqrt(((pShots.get(i).getX() - pShotsInitial.get(i).getX())
                                    * (pShots.get(i).getX() - pShotsInitial.get(i).getX()))
                                    + ((pShots.get(i).getY() - pShotsInitial.get(i).getY())
                                    * (pShots.get(i).getY() - pShotsInitial.get(i).getY()))) < 500)) {
                        pShotsInitial.remove(i);
                        pShots.remove(i);

                        elixirBoxes.get(j).decreaseHealth(pam.getDamage());
                        if (elixirBoxes.get(j).getHealth() <= 0) {
                            Gem g = new Gem(elixirBoxes.get(j).getX(), elixirBoxes.get(j).getY());
                            gems.add(g);
            ...
            for (int i = 0; i < pShots.size(); i++) {
                if ((!pShots.get(i).checkCollisionColt(colt)) && (pShots.get(i).checkMove(blocksX, blocksY)
                        && (Math.sqrt(((pShots.get(i).getX() - pShotsInitial.get(i).getX())
                                * (pShots.get(i).getX() - pShotsInitial.get(i).getX()))
                                + ((pShots.get(i).getY() - pShotsInitial.get(i).getY())
                                * (pShots.get(i).getY() - pShotsInitial.get(i).getY()))) < 500))) {
                    ...
                    pSuper += 5;
                    colt.decreaseHealth(pam.getDamage());
                    pShotsInitial.remove(i);
                    pShots.remove(i);
            ...
            for (int i = 0; i < cShots.size(); i++) {
                for (int j = 0; j < elixirBoxes.size(); j++) {
                    if ((i < cShots.size()) && cShots.get(i).checkCollisionBox(elixirX.get(j), elixirY.get(j))
                            && (Math.sqrt(((cShots.get(i).getX() - cShotsInitial.get(i).getX())
                                    * (cShots.get(i).getX() - cShotsInitial.get(i).getX()))
                                    + ((cShots.get(i).getY() - cShotsInitial.get(i).getY())
                                    * (cShots.get(i).getY() - cShotsInitial.get(i).getY()))) < 500)) {
                        cShotsInitial.remove(i);
                        cShots.remove(i);

                        elixirBoxes.get(j).decreaseHealth(colt.getDamage());
                        if (elixirBoxes.get(j).getHealth() <= 0) {
                            Gem g = new Gem(elixirBoxes.get(j).getX(), elixirBoxes.get(j).getY());
                            gems.add(g);
                            elixirBoxes.remove(j);
            ...
            for (int i = 0; i < cShots.size(); i++) {
                if ((!cShots.get(i).checkCollisionPam(pam)) && (cShots.get(i).checkMove(blocksX, blocksY)
                        && (Math.sqrt(((cShots.get(i).getX() - cShotsInitial.get(i).getX())
                                * (cShots.get(i).getX() - cShotsInitial.get(i).getX()))
                                + ((cShots.get(i).getY() - cShotsInitial.get(i).getY())
                                * (cShots.get(i).getY() - cShotsInitial.get(i).getY()))) < 500))) {
                    ...
                    cSuper += 7;
                    pam.decreaseHealth(colt.getDamage());
                    cShotsInitial.remove(i);
                    cShots.remove(i);
            ...
            for (int i = 0; i < cSupers.size(); i++) {
                for (int j = 0; j < elixirBoxes.size(); j++) {
                    if ((i < cSupers.size()) && cSupers.get(i).checkCollisionBox(elixirX.get(j), elixirY.get(j))
                            && (Math.sqrt(((cSupers.get(i).getX() - cSuperInitial.get(i).getX())
                                    * (cSupers.get(i).getX() - cSuperInitial.get(i).getX()))
                                    + ((cSupers.get(i).getY() - cSuperInitial.get(i).getY())
                                    * (cSupers.get(i).getY() - cSuperInitial.get(i).getY()))) < 500)) {
            ...
            for (int i = 0; i < cSupers.size(); i++) {
                ...
                if ((!cSupers.get(i).checkCollisionPam(pam)) && (cSupers.get(i).checkMove(blocksX, blocksY, size)
                        && (Math.sqrt(((cSupers.get(i).getX() - cSuperInitial.get(i).getX())
                                * (cSupers.get(i).getX() - cSuperInitial.get(i).getX()))
                                + ((cSupers.get(i).getY() - cSuperInitial.get(i).getY())
                                * (cSupers.get(i).getY() - cSuperInitial.get(i).getY()))) < 630))) {
                    ...
                    pam.decreaseHealth(colt.getSuperdamage());
            ...
            while (gemscount < gems.size()) {
                if (gems.get(gemscount).checkCollision(colt)) {
                    colt.setGems(colt.getGems() + 1);
                    colt.setDamage((int) (colt.getDamage() * 1.1));
                    colt.setSuperdamage((int) (colt.getSuperdamage() * 1.1));
